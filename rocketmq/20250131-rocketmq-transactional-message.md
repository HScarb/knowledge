...
![](./20250131-rocketmq-transactional-message.assets/202501302328202.png)

1. 生产者（Sender）发送消息到 RocketMQ 服务端（Server），除了原本的信息外，这个消息的属性中添加了事务消息标识，是一个事务半消息（Half message）。
2. 事务半消息会被保存在服务端一个专门用来保存半消息的特殊 Topic 中，消息持久化成功之后，向生产者返回 Ack 确认消息已经发送成功。由于在特殊 Topic 中，所以此时事务半消息无法被消费者消费到。
3. 生产者开始执行本地事务逻辑。
4. 生产者如果已经执行完本地事务，那么根据本地事务执行结果向服务端提交二次确认结果（Commit/Rollback）。如果本地事务没有执行完毕，这里的本地事务执行结果可以返回 Unknown，稍后由服务端进行本地事务执行结果的回查。服务端收到确认结果后处理逻辑如下：
   - 二次确认结果为 Commit：服务端将事务半消息复原，发送到真实的 Topic，可以投递给消费者。
   - 二次确认结果为 Rollback：服务端将回滚事务，不会将事务半消息投递给消费者。
   - 二次确认结果为 Unknown：暂不处理，服务端在一定时间后，对生产者实例发起定时回查。
5. 在断网或者是生产者应用重启的特殊情况下，若服务端未收到发送者提交的二次确认结果；或服务端收到的二次确认结果为 Unknown 未知状态，经过一定时间后，服务端将对生产者集群中任一生产者实例发起定时回查。如果超过了最大回查次数，则将事务半消息丢弃。
6. 生产者收到消息回查请求后，需要检查对应消息的本地事务执行的最终结果。
